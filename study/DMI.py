...
def wirte_by_type(type_start,type_end,rd_book,new_l,xls_type_line_sn,x, style):
...
def wirte_by_repeat_type(type_start,type_end, rd_book, new_l, xls_type_line_sn, x, style):
    rd_book = xlrd.open_workbook(file_path, formatting_info=True)
    for j in range(xls_type_line_sn[type_start], xls_type_line_sn[type_end]):
        if rd_book.sheet_by_index(0).cell(j, 1).value == new_l[i][0]:
            x = x + 1
...
xls_type_line_sn = {'Type 000': 1, 'Type 001': 11, 'Type 002': 22, 'Type 003': 32, 'Type 004': 41, 'Type 007': 66,
                    'Type 009': 77, 'Type 010': 88, 'Type 011': 93, 'Type 016': 110, 'Type 017': 120, 'Type 019': 148,
...
for i in range(len(new_l)):
    if i< type_line_sn['Type 000']:
        ...
        wirte_by_type('Type 001', 'Type 002', rd_book, new_l, xls_type_line_sn, x, style)
    elif type_line_sn['Type 002'] < i < type_line_sn['Type 003']:
        wirte_by_type('Type 002', 'Type 003', rd_book, new_l, xls_type_line_sn, x, style)
    elif type_line_sn['Type 003'] < i < type_line_sn['Type 004']:
        wirte_by_type('Type 003', 'Type 004', rd_book, new_l, xls_type_line_sn, x, style)
    elif type_line_sn['Type 004'] < i < type_line_sn['Type 007']:
        wirte_by_type('Type 004', 'Type 007', rd_book, new_l, xls_type_line_sn, x, style)
        ...
        wirte_by_type('Type 016', 'Type 017', rd_book, new_l, xls_type_line_sn, x, style)
    elif type_line_sn['Type 019'] < i < type_line_sn['Type 020']:
        wirte_by_type('Type 019', 'Type 020', rd_book, new_l, xls_type_line_sn, x, style)
    elif type_line_sn['Type 024'] < i < type_line_sn['Type 032']:
        wirte_by_type('Type 024', 'Type 032', rd_book, new_l, xls_type_line_sn, x, style)
    elif type_line_sn['Type 032'] < i < type_line_sn['Type 041']:
        wirte_by_type('Type 032', 'Type 041', rd_book, new_l, xls_type_line_sn, x, style)
    elif type_line_sn['Type 043'] < i < len(new_l):
        wirte_by_type('Type 043', 'end', rd_book, new_l, xls_type_line_sn, x, style)
    elif type_line_sn['Type 007'] < i < type_line_sn['Type 009']:
        if len(new_l[i]) == 1:
            continue
        else:
            wirte_by_repeat_type('Type 007', 'Type 009', rd_book, new_l, xls_type_line_sn, x, style)
            pass
    elif type_line_sn['Type 009'] < i < type_line_sn['Type 010']:
        if len(new_l[i]) == 1:
            continue
        else:
            wirte_by_repeat_type('Type 009', 'Type 010', rd_book, new_l, xls_type_line_sn, x, style)
            pass
    elif type_line_sn['Type 010'] < i < type_line_sn['Type 011']:
        if len(new_l[i]) == 1:
            continue
        else:
            wirte_by_repeat_type('Type 010', 'Type 011', rd_book, new_l, xls_type_line_sn, x, style)
            pass
    elif type_line_sn['Type 017'] < i < type_line_sn['Type 019']:
        if len(new_l[i]) == 1:
            continue
        else:
            wirte_by_repeat_type('Type 017', 'Type 019', rd_book, new_l, xls_type_line_sn, x, style)
            pass
    elif type_line_sn['Type 020'] < i < type_line_sn['Type 024']:
        if len(new_l[i]) == 1:
            continue
        else:
            wirte_by_repeat_type('Type 020', 'Type 024', rd_book, new_l, xls_type_line_sn, x, style)
            pass
    elif type_line_sn['Type 041'] < i < type_line_sn['Type 043']:
        if len(new_l[i]) == 1:
            continue
        else:
            wirte_by_repeat_type('Type 041', 'Type 043', rd_book, new_l, xls_type_line_sn, x, style)
            pass
